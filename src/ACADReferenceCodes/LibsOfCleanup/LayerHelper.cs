...
namespace CADAddins.LibsOfCleanup
{
    public class LayerHelper
    {
        ...
        private Dictionary<string, List<string>> _dirtytypes;
...
//            MessageBox.Show($"\n{nameof(_dictOffSpecial)}:{string.Join(",",_dictOffSpecial.Values)}");
               dynamic blks = _curDocHelper.BlockTableId;
            using (Transaction trans = _curDocHelper.StartTransaction())
            {
        ...
        public void CleanupEnts()
        {
                var layers = _layerTableId;
                _cleantypes = new Dictionary<string, dynamic>();
                List<dynamic> layersToClean = new List<dynamic>();
...
//                    MessageBox.Show(layer.Name);
                ...
                foreach (dynamic layer in layersToClean)
                {
        ...
        {
            string layerName = layer.Name;
            dynamic ltype = layer.LinetypeObjectId;
            string ltypeName = ltype.Name;
            MakeEntsLtypesBylayer(layerName, ltypeName);
            string layerShtName=GetShtName(layerName, ltypeName);
        ...
        {
            string ltypeName = ent.Linetype;
            string ltypeShtName = GenUtils.RemoveBoundPrefix(ltypeName);
            string tgtLayerName = GetNewName(layerShtName, ltypeShtName);
        ...
        private void MakeEntsLtypesBylayer(string layerName, string ltypeName)
    {
        ...
        private void DelSpecialFrozenNpltLayers()
        {
            ...
            using (Transaction trans = _curDocHelper.StartTransaction())
            {
                ...
                while (ge.MoveNext())
                {
                    string layerName = ge.Current.Key;
        ...
        private void MergeDirtyLayers()
        {
            ...
            {    var ge = _dirtytypes.GetEnumerator();
                while (ge.MoveNext())
                {
                    string newName = ge.Current.Key;
                    List<string> layers = ge.Current.Value;
        ...
        {
            List<string> cmdlist = new List<string> { "-laymrg" };
        ...
        {
            List<string> cmdlist = new List<string> {"-laydel"};
        ...
        private void AddToDirtyDict(string layerName, string ltypeName, dynamic layer)
        {
            ...
            string newName = GetNewName(layerName, ltypeName);
            List<string> dirtyLayers;
            if (!_cleantypes.ContainsKey(newName.ToUpper()))
            {
                if (string.Equals(layerName, newName, StringComparison.CurrentCultureIgnoreCase)|| Rename(layer, newName) != null)
                {
        ...
        private static string GetNewName(string layerName, string ltypeName)
        {
            ...
            string shortName = GenUtils.RemoveBoundPrefix(layerName).ToUpper();
            string newName = string.Join(_layerSep, new List<string> {ltypeName, shortName});
            return newName;
        }
        ...
        public string DelNonBlkEntsOnOffLayer(dynamic layer)
        {
            ...
            dynamic ents = _curEditorHelper.SelectNoneBlkOnLayer(layerName);
            if (ents != null)
            {
                O_EntExt.EraseEnts(ents);
        ...
        {
            return string.Join(_layerSep, new string[] {layerName, "OFF"});
        }
