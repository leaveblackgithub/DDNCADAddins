...
//            CleanupTextStypesInDimStyles();
        ...
        {
            dynamic styles = _curDocHelper.MLeaderStyleDictionaryId;
            foreach (dynamic item in styles)
            {
                dynamic style = item.Value;
                dynamic txtStyle = style.TextStyleId;
        ...
        {
            dynamic dimStyles = _curDocHelper.DimStyleTableId;
            foreach (dynamic dimStyleId in dimStyles)
            {
            ...
            foreach (var blkId in blks)
                using (var trans = _curDocHelper.StartTransaction())
                {
                    ...
                    foreach (dynamic objId in blk)
                    {
                        ...
                        if (GenUtils.ContainProperty(obj, NameOfTextStyleId))
                        //                            ||
...
//                            trans.GetObject(objId, OpenMode.ForWrite);
                            ...
                            if (!DirtyDict.TryGetValue(idString, out dynamic newTextStyle)) continue;
                            try
                            {
