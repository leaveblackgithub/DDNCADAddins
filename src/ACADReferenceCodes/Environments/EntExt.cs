...
namespace CADAddins.Environments
{
    ...
    public static class EntExt
    {
        ...
        public static PointContainment GetPointContainment(this Curve curve, Point3d point)
        {
            if (!curve.Closed)
                throw new ArgumentException("Curve must be closed.");
            Region region = EntExt.RegionFromClosedCurve(curve);
            if (region == null)
                throw new InvalidOperationException("Failed to create region");
            using (region)
            {
                return EntExt.GetPointContainment(region, point);
            }
        }
        ...
        {
            int ptCount = pt3dCollection.Count;
            double[] paraArr = new double[ptCount];
            for (int i = 0; i < ptCount; i++)
            {
                paraArr[i] = curve.GetParameterAtPoint(pt3dCollection[i]);
            }
...
//        }
        ...
        {
            Point3dCollection ptInts = new Point3dCollection();
        ...
        {
            double[] arr1 = pt1.ToArray();
            double[] arr2 = pt2.ToArray();
            double[] arrResult=new double[3];
            for (int i = 0; i <= 2; i++)
            {
                arrResult[i] = (arr1[i] + arr2[i]) / 2;
            }
        ...
        public static IEnumerable<ObjectId> GenerateHatchBoundaries(this Hatch hatch,CommandTransBase cmdTransBase)
        {
            ...
            for (var i = 0; i < nLoops; i++)
            {
                ...
                {
                    boundaryIds.Add(cv.Curve2DToDbCurve(plane, ownerId, cmdTransBase));
                }
            }
